...
def get_sequences(file_name, _format):
    ...
    return list(set(all_seq)) # unique seq
...
def find_unique(sequences, pattern):
    ...
    return list(set(possible_hits)) # matches of the regex
...
def rearrangement(sequences_28):
    ...
    return to_seek  # list of queries 12N+TA+12N
...
FORMAT = r'AGACCGGGGACTTATCATCCAACCTGT.{28}ACAGGTTGGATGATAAGTCCCCGGTCT'  # in our case IR + 28N + IR
...
quary_reads = r1_to_neg + r1_to_pos + r2_to_neg + r2_to_pos # to pool 
match_list = list()   
...
gene_hits = list(set(match_list))  # initial results
...
for locus in mabs_x:  # find the full sequence of the harmed gene 
...
ncr_r = init_mapper(test2, genome_neg)  #  results of non-coding seq